...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            //    PHP for Payment Data Search  //  
            elseif (isset($_POST["Payment"])) {
                // Get the selected artist ID
                $artist_id = $_POST["ArtistID"];
                // Retrieve payment data based on the selected artist ID
                $payments_query = $conn->prepare("SELECT APaymentDate, Amount FROM artistpayment_t WHERE ArtistID = $artist_id");
                $payments_query->execute();
        
                // Display the payment data in a table
                if ($payments_query->rowCount() > 0) {
                    echo "<table>";
                    echo "<tr><th>Name</th><th>Payment Date</th><th>Amount</th></tr>";
                    $name_query = $conn->prepare("SELECT FirstName FROM artist_t WHERE ArtistID =  $artist_id");
                    $name_query->execute();
                    $name = $name_query->fetchColumn();
                    while ($row = $payments_query->fetch()) {
                        $payment_date = $row['APaymentDate'];
                        $amount = $row['Amount'];
                        echo "<tr><td>$name</td><td>$payment_date</td><td>$amount</td></tr>";
                    }
                    echo "</table>";
                } else {
                    echo "No payment records found for this artist.";
                }
            }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (isset($_POST['submit'])) {
            ...
            // Generate forms for each column
            $forms = "";
            ...
            // Generate SET statement for update query
            $set = "";
            foreach($columns as $column) {
    ...
    <div class="Tmenu">
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            <label for="table">Select a Artist Table To View:</label>
